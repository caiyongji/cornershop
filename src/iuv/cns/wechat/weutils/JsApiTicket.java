...
public class JsApiTicket {
	private final static Log LOG = LogFactory.getLog(JsApiTicket.class);
	...
	public static String ticket() {
		...
		if (ticketIsUseful) {
			...
			if (successlyGetJsApiTicketFromWe()) {
				return TICKET;
			} else {
				// LOG.error("获取TICKET失败");
				// // 出事啦！给蔡永吉发邮件吧~~
				if (!MailUtil.sendMail("【JSAPITICKET】出事啦", "请在【"
	 ...
	 * @return是否成功
	 */
	private static synchronized boolean successlyGetJsApiTicketFromWe() {
		boolean ticketIsUseful = StringUtil.isValid(TICKET)
				&& EXPIRESIN - (Calendar.getInstance().getTimeInMillis() - TIMESTAMP) / 1000 > Constants.EXPIRES_REMAIN;
		if (ticketIsUseful) {
			return true;
			...
			String result = "";
			try {
				result = ConnectWeChatTo.getJsTicket();
				JSONObject json = JSONObject.fromObject(result);
				EXPIRESIN = json.getInt("expires_in");
				TIMESTAMP = Calendar.getInstance().getTimeInMillis();
				// 最后获取，不会因为上方报错而更改可用TICKET
				TICKET = json.getString("ticket");
				LOG.info("【获取JSAPITICKET】" + DateUtil.getTimeStampString() + "--JSAPITICKET:【" + TICKET
						+ "】expires_in:【" + EXPIRESIN + "】秒");
			} catch (Exception e) {
				LOG.error("获取【JSAPITICKET】失败", e);
				return false;
			}
			return true;
		}
